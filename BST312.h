...
{
    if (t!=NULL){ //goes through and deletes the item at the node
        makeEmpty(t->left);
        makeEmpty(t->right);
        delete t;
    }
...
{
    TreeNode *temp = new TreeNode;
    if (t == NULL){ //if empty there add in new item
        temp -> data = newItem;
        temp -> left = NULL;
        temp -> right = NULL;
        t = temp;
    }
    else if (t->data < newItem){ //if not go to new node
...
{
    if(t!=NULL){
        return countNodes(t->left)+countNodes(t->right)+1;//go through each node and count if there
    }
    else{
        return 0;
    }
...
{
    if (t != NULL){
        result.push_back(t->data);//node first then down left then down right
...
{
    if(t!=NULL){
        inOrderTraversal(t->left, result);//all the way down left then record data then all the way down right and record data
...
{
    if(t!=NULL){
        postOrderTraversal(t->left, result); //will go down to the left, then to the right all the way and record the data at the node
...
{
    vector<ItemType> results;
    postOrderTraversal(root, results); //gives vector full of data
    for(int i = 0; i <=results.size(); i++){ //go through vector and compare to item to see if in tree
        if(results[i] == item){
            //cout << "this worked";
            return true;
        }
    }
    //cout << "this didnt work";
    return false;
}
